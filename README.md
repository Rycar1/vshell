...
| **Agent 999-function full mapping** | **632 functions mapped** (`.re/decomp/agent_map_*.txt`): full pipeline decoded — main state machine 0xfb1440 → decode → expression engine → executor (182-opcode 0x101df40 + 0x155-opcode 0x10f3220) → 24B frame builder → transport (pooled conn 0xfc7560) → session CRUD → result send; SQLite driver + garble string pool (0xf50000-0xfb0000), script interpreter chain (lexer 0x10f9bc0 → loop 0x10f8e40 → core 0x10f3220 → executor 0x10fa480), plugin subsystem (dispatch 0x1153100 → task exec 0x1152dc0). Architecture: `.re/AGENT_ARCHITECTURE.md` | Substantial |
